...
def apply_post_build_patches(frida_dir: Path, custom_name: str):
    """Patch identifiers generated by Vala during the first build.

    Must include build/ directory because:
    - frida_agent_main: agent-glue.c (source) CALLS it; meson-generated_agent.c
      (build output) DEFINES it. Both must be renamed together or link fails.
    - frida-error-quark: Vala autogenerates `frida_error_quark()` for the
      `Frida.Error` enum and emits `g_quark_from_static_string("frida-error-quark")`
      inside it. The string literal lives only in generated C, so source-tree
      patches in PHASE 1 cannot reach it.

    Both renames are applied to source AND build/, then the second incremental
    build recompiles whatever was touched. Length differences are absorbed by
    recompilation.
    """
    log("PHASE 3: Post-build patches (Vala-generated identifiers)...", "STEP")
    ...
    count = replace_in_tree(frida_dir, "frida-error-quark",
                            f"{custom_name}-error-quark", include_build=True)
    log(f"  frida-error-quark -> {custom_name}-error-quark ({count})", "OK")
