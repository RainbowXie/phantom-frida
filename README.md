...
Not yet covered (the floor for rename-only stealth — going lower breaks compatibility):

- Protocol-bound D-Bus identifiers: `/re/frida/HostSession`, `re.frida.AgentSession17`, `frida:rpc`, etc. These travel on the wire between the standard `frida` client and the server; renaming server-side requires forking the client too.
- `frida-context` GObject property name (auto-generated by Vala from the `frida_context` identifier on `Frida.Agent` / payload classes). Renaming would mean either renaming the Vala identifier across every payload site or renaming the `Frida.*` namespace tree — both are deep refactors.
